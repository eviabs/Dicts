...
public class SettingsFragment extends PreferenceFragmentCompatDividers {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, @Nullable Bundle savedInstanceState) {
        try {
            ...
            for (final String dictionary: ApiConsts.SEARCH_PROVIDERS) {
                CheckBoxPreference dictionaryPreference = new CheckBoxPreference(getContext());
                dictionaryPreference.setDefaultValue(true);
                dictionaryPreference.setIcon(null);
                dictionaryPreference.setKey(dictionary);
                dictionaryPreference.setTitle(dictionary);
                categoryCustomSearch.addPreference(dictionaryPreference);
                dictionaryPreference.setDependency("custom_search_providers");

                dictionaryPreference.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                    @Override
                    public boolean onPreferenceChange(Preference preference, Object newValue) {
                        if (!(Boolean) newValue) {
                            ((MainActivity) getActivity()).getLocalPreferences().removeSearchProviders(dictionary);
                        } else {
                            ((MainActivity) getActivity()).getLocalPreferences().addSearchProviders(dictionary);
                        }
